...
## Interview Question Behind This Code

> **“Node.js is single-threaded. Then how can one server handle thousands of requests?”**

---

## First: What *NOT* to think ❌

❌ **Wrong mental model**

> “Node creates a new thread per request.”

This is **Java / Spring thinking**, not Node.
...
## Correct Interview Mental Model ✅

### Step-by-step thinking (this is what interviewer wants)

### 1️⃣ One JavaScript thread exists
...
So yes — **Node is single-threaded**.

---

### 2️⃣ Requests are NOT processed fully on the JS thread
...
Node does **very little work** here:

* parses request
* registers a callback
* delegates work if needed

---

### 3️⃣ Heavy / slow tasks are pushed out

When Node hits something slow:
...
JS Thread
   ↓
Delegate task to OS / libuv
   ↓
Continue handling next request
```

---

### 4️⃣ Event Loop does the coordination

Think of the event loop like a **traffic manager**:
...
Event Loop
      ↓
JS Thread (one at a time)
```

Only the **final callback** comes back to JS thread.

---

### 5️⃣ Thousands of requests ≠ thousands of threads

Thousands of requests usually mean:

* many requests are **waiting on I/O**
* very little JS execution per request
* callbacks are short and fast

So one thread can handle:

```
Request A → waiting for DB
Request B → waiting for file
Request C → waiting for API
```

JS thread stays **free**, not blocked.

---

## Apply this thinking to YOUR code
...
1. Request comes in
2. Node registers callback
3. Response is tiny and immediate
4. JS thread finishes quickly
5. Next request is handled

⚡ Extremely fast, minimal blocking

---

## Key Interview Sentence (MEMORIZE THIS)

> **“Node.js uses a single-threaded, event-driven, non-blocking I/O model, which allows it to handle thousands of concurrent requests efficiently.”**
...
> “Then when does Node fail?”

### Correct answer:

> “Node struggles with CPU-intensive tasks because they block the single JavaScript thread.”
...
## Bonus: How Node scales further in real apps
...
```
CPU Core 1 → Node Process
CPU Core 2 → Node Process
CPU Core 3 → Node Process
```

Still **each process is single-threaded**, but system scales.

---

## Final Interview Flow (say this calmly)

1. Node is single-threaded for JS execution
2. It uses non-blocking I/O
3. Heavy tasks are handled by OS/libuv
4. Event loop manages callbacks
5. Thousands of concurrent requests are possible
6. CPU-heavy tasks are the limitation
